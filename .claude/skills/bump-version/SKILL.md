---
name: bump-version
description: Bump ender_chest_drop manifest.json (header + module version) and package.json together, and report the tag for release.yml
disable-model-invocation: true
---

# Bump Version

Keeps `ender_chest_drop/manifest.json` (`header.version`, `modules[0].version`) and `package.json`
...
Steps:
1. Read the current `header.version` from `ender_chest_drop/manifest.json`.
...
4. Update both `header.version` and `modules[0].version` in `ender_chest_drop/manifest.json` to the
